...
fn load_recovers_wallet() -> anyhow::Result<()> {
    ...
    where
        B: Persist<bdk_wallet::wallet::ChangeSet> + Send + Sync + 'static,
        ...
        FR: Fn(&Path) -> anyhow::Result<B>,
    {
        ...
        let wallet_spk_index = {
            let mut wallet = Wallet::new(get_test_tr_single_sig_xprv(), None, Network::Testnet)
...
fn new_or_load() -> anyhow::Result<()> {
        ...
        F: Fn(&Path) -> anyhow::Result<B>,
    {
        ...
        let wallet_keychains: BTreeMap<_, _> = {
            let wallet = &mut Wallet::new_or_load(get_test_wpkh(), None, None, Network::Testnet)
                .expect("must init wallet");
            let staged_changeset = wallet.take_staged();
            let db = &mut new_or_load(&file_path).expect("must create db");
            db.write_changes(&staged_changeset)
                .map_err(|e| anyhow!("write changes error: {}", e))?;
        ...
        // wrong network
        {
            ...
            let err = Wallet::new_or_load(get_test_wpkh(), None, changeset, Network::Bitcoin)
        ...
        // wrong external descriptor
        {
            ...
            let changeset = db
                .load_changes()
                .map_err(|e| anyhow!("load changes error: {}", e))?;
            let err = Wallet::new_or_load(exp_descriptor, None, changeset, Network::Testnet)
        ...
        // wrong internal descriptor
        {
            ...
            let changeset = db
                ...
                .map_err(|e| anyhow!("load changes error: {}", e))?;
            let err =
                Wallet::new_or_load(get_test_wpkh(), exp_descriptor, changeset, Network::Testnet)
        ...
        // all parameters match
        {
            ...
            let changeset = db
                .load_changes()
                .map_err(|e| anyhow!("load changes error: {}", e))?;
            let wallet = Wallet::new_or_load(get_test_wpkh(), None, changeset, Network::Testnet)
                .expect("must recover wallet");
            assert_eq!(wallet.network(), Network::Testnet);
            assert!(wallet
                .keychains()
...
fn test_get_address() {
    ...
    let wallet = Wallet::new(Bip84(key, KeychainKind::External), None, Network::Regtest).unwrap();
...
fn test_reveal_addresses() {
    ...
    let mut wallet = Wallet::new(desc, None, Network::Signet).unwrap();
